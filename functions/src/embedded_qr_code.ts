...
export const scan3SecondGen = onRequest(
  { cors: true },
  async (request, response) => {
    console.log(request.body);
    ...
    const totemId = data.totemId;
    const requestId = data.requestId;
    const userHasAlreadyScannedThisTotemForLastSessionScanned =
      data.userHasAlreadyScannedThisTotemForLastSessionScanned;
    ...
    const userHasAlreadySignedOnActiveSession =
      (lastSessionIdScanned != null &&
        lastSessionIdScanned == eventData.activeSessionId) ||
      (v1 && scannedSessions[eventData.activeSessionId] != null);

    const mutexTotemsEnabled = eventData.mutexTotemsEnabled ?? true;

    if (userHasAlreadySignedOnActiveSession) {
      if (mutexTotemsEnabled) {
        response.status(200).send({
          status: "sessionAlreadyScanned",
          sessionId: eventData.activeSessionId,
        });
        return;
      } else if (userHasAlreadyScannedThisTotemForLastSessionScanned) {
        response.status(200).send({
          status: "sessionAlreadyScannedForThisTotem",
          sessionId: eventData.activeSessionId,
        });
        return;
      }
    ...
    if (mutexTotemsEnabled || !userHasAlreadySignedOnActiveSession) {
      const batch = db.batch();
      const userSubEventDocRef = sessionDocRef(
        providerId,
        eventId,
