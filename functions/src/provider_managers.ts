...
export const confirmPendingInviteSecondGen = onRequest(
  { cors: true },
  async (request, response) => {
    ...
    if (secretOnDb !== secret) {
      ...
      throw new HttpsError("permission-denied", "The secret is different!");
    }
    ...
    if (userId === undefined || userId === null) {
      ...
      console.log("get existing user " + userId + " of provider " + providerId);
...
export const onInviteToCollaborate = onDocumentCreated(
  "providers/{providerId}/pendingInvite/{inviteId}",
  async (event) => {
    ...
    if (snap == null) {
      console.log("onInviteToCollaborate event data null");
      return;
    }
